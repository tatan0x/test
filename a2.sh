...
declare -r EFI_LABEL="EFI"
declare -r BTRFS_LABEL="arch"
declare -r SWAP_LABEL="Swap"
...
declare -ar CORE_PACKAGES=(
    ...
    "wireless-regdb" "lm_sensors" "smartmontools" "timeshift"
)
...
_main_installer() {
    ...
    _prompt_input HOST_NAME "desired Hostname" "archlinux"
    _prompt_input DEFAULT_USER "desired Username (lowercase, no spaces)" "user"
...
size=1GiB,type=C12A7328-F81F-11D2-BA4B-00A0C93EC93B,name="${EFI_LABEL}"
type=0FC63DAF-8483-4772-8E79-3D69D8477DE4,name="${BTRFS_LABEL}"
EOF
    ...
    mkfs.fat -F32 -n "${EFI_LABEL}" "${efi_part}" || _error "mkfs.fat failed on ${efi_part}"
    mkfs.btrfs -f -L "${BTRFS_LABEL}" "${btrfs_part}" || _error "mkfs.btrfs failed on ${btrfs_part}"
    ...
    chmod 0600 "${swap_file}" && mkswap -L "${SWAP_LABEL}" "${swap_file}" && swapon "${swap_file}"
    ...
    declare -ar services_to_enable=(
        ...
        "smartd.service" "systemd-timesyncd.service"
    )
    ...
    _chroot "echo '%wheel ALL=(ALL:ALL) ALL' > /etc/sudoers.d/${DEFAULT_USER} && chmod 0440 /etc/sudoers.d/${DEFAULT_USER}"
